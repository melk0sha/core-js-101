...
function isInsideCircle(circle, point) {
  return (point.x - circle.center.x) ** 2 + (point.y - circle.center.y)
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let str = '';
  if (isStartIncluded && a > b) {
    str = isEndIncluded ? `[${b}, ${a}]` : `[${b}, ${a})`;
  } else if (isStartIncluded && b > a) {
    str = isEndIncluded ? `[${a}, ${b}]` : `[${a}, ${b})`;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (brackets[str[i]] === str[i] && stack[stack.length - 1] === str[i]) {
      stack.pop();
...
function getCommonDirectoryPath(pathes) {
  let minimalLength = pathes[0].length;
  pathes.forEach((path) => {
    if (minimalLength > path.length) {
      minimalLength = path.length;
    }
  });
  let result = '';
  let flag = false;
  for (let i = 0; i < minimalLength; i += 1) {
    const symbolForCheck = pathes[0][i];
    for (let j = 0; j < pathes.length; j += 1) {
      // pathes.forEach((path) => {
      if (symbolForCheck !== pathes[j][i]) {
        flag = true;
      }
    // });
    }
    if (!flag) {
      result += symbolForCheck;
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  return result.map((x) => x.filter((y) => y));
}
