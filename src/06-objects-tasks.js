...
class CssSelector {
  constructor() {
    this.selector = '';
    this.type = 0;
    this.oneTimeInsideError = 'Element, id and pseudo-element should not occur more then one time inside the selector';
    this.correctQueueError = 'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element';
  }
  ...
  checkType(type) {
    if (type < this.type) {
      throw new Error(this.correctQueueError);
    }
    if (type === this.type && [1, 2, 6].includes(type)) {
      throw new Error(this.oneTimeInsideError);
    }
    this.type = type;
  }
}
